package com.epam.hospital.controller.command;

public class CommandName {
    public static final String GOTO_HOME_PAGE_COMMAND = "goto-home-page";
    public static final String GOTO_PATIENTS_PAGE_COMMAND = "goto-patients-page";
    public static final String GOTO_PROCEDURES_PAGE_COMMAND = "goto-procedures-page";
    public static final String GOTO_DEPARTMENTS_PAGE_COMMAND = "goto-departments-page";
    public static final String GOTO_DOCTORS_PAGE_COMMAND = "goto-doctors-page";
    public static final String GOTO_NEWS_PAGE_COMMAND = "goto-news-page";
    public static final String GOTO_CONTACTS_PAGE_COMMAND = "goto-contacts-page";
    public static final String GOTO_LOG_IN_PAGE_COMMAND = "goto-log-in-page";
    public static final String GOTO_SIGN_UP_PAGE_COMMAND = "goto-sign-up-page";
    public static final String GOTO_MANAGE_PAGE_COMMAND = "goto-manage-page";
    public static final String GOTO_ADMIN_CHAMBERS_COMMAND = "goto-admin-chambers-page";
    public static final String GOTO_ADMIN_PERSONAL_COMMAND = "goto-admin-personal-page";
    public static final String GOTO_DEPARTMENT_CHAMBERS_PAGE_COMMAND = "goto-department-chambers-page";
    public static final String GOTO_DEPARTMENT_PERSONAL_PAGE_COMMAND = "goto-department-personal-page";
    public static final String GOTO_NEW_PATIENT_PAGE_COMMAND = "goto-new-patient-page";
    public static final String GOTO_SEARCH_PATIENT_PAGE_COMMAND = "goto-search-patient-page";
    public static final String GOTO_NEW_PROCEDURE_PAGE_COMMAND = "goto-new-procedure-page";
    public static final String GOTO_SEARCH_PROCEDURE_PAGE_COMMAND = "goto-search-procedure-page";
    public static final String GOTO_ACCOUNT_PROFILE_PAGE_COMMAND = "goto-account-profile-page";
    public static final String GOTO_ACCOUNT_SETTINGS_PAGE_COMMAND = "goto-account-settings-page";
    public static final String GOTO_PROFILE_PAGE_COMMAND = "goto-profile-page";
    public static final String PROFILE_CHANGE_COMMAND = "profile-change-page";
    public static final String CREATE_NEW_PATIENT_COMMAND = "create-new-patient";
    public static final String CREATE_NEW_PROCEDURE_COMMAND = "create-new-procedure";
    public static final String SEARCH_PATIENT_COMMAND = "search-patient";
    public static final String SEARCH_PROCEDURE_COMMAND = "search-procedure";
    public static final String GOTO_403_PAGE_COMMAND = "goto-403-page";
    public static final String GOTO_404_PAGE_COMMAND = "goto-404-page";
    public static final String GOTO_500_PAGE_COMMAND = "goto-500-page";
    public static final String GOTO_EDIT_PERSONAL_DATA_PAGE_COMMAND = "goto-edit-personal-data-page";
    public static final String LOG_IN_COMMAND = "log-in";
    public static final String LOG_OUT_COMMAND = "log-out";
    public static final String SET_LOCALE_COMMAND = "set-locale";
    public static final String SIGN_UP_COMMAND = "sign-up";

    private CommandName() {
    }
}