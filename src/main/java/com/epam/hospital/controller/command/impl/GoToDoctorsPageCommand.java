...
public class GoToDoctorsPageCommand implements Command {
    @Override
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        session.setAttribute(SessionAttribute.URL, "/controller?" +
                RequestParameter.COMMAND + "=" + CommandName.GOTO_DOCTORS_PAGE_COMMAND);

        return new CommandResult(Page.DOCTORS_PAGE, RoutingType.FORWARD);
    }
}
