...
public class GoToAccountSettingsPageCommand implements Command {
    @Override
    public CommandResult execute(HttpServletRequest request, HttpServletResponse response) {
        HttpSession session = request.getSession();
        session.setAttribute(SessionAttribute.URL, "/controller?" +
                RequestParameter.COMMAND + "=" + CommandName.GOTO_ACCOUNT_SETTINGS_PAGE_COMMAND);

        return new CommandResult(Page.ACCOUNT_SETTINGS_PAGE, RoutingType.FORWARD);
    }
}
